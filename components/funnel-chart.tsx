...
export function FunnelChart({ data }: { data: FunnelData }) {
  return (
    <div
      className={`relative flex h-full flex-col overflow-hidden ${panelClass("dark")}`}
      style={{ ...panelStyle("dark"), containerType: "inline-size" }}
    >
      <div className="relative mb-5 flex flex-wrap items-center justify-between gap-2">
        ...
        <h3 className="font-display flex items-center gap-2 text-[clamp(15px,2.8cqw,19px)] font-bold text-primary">
          <FunnelIcon className="text-accent-light drop-shadow-[0_0_6px_rgba(58,67,227,0.6)]" />
          Funil de Conversão
        </h3>
        <div
          className="inline-flex items-center gap-1 rounded-none border px-3.5 py-2 text-[clamp(11px,2cqw,14px)]"
          style={{
            borderColor: "rgba(88,141,255,0.45)",
            background: "rgba(13,40,75,0.65)",
            color: "#b8c8db",
            boxShadow: "inset 0 0 12px rgba(58,67,227,0.1), 0 0 16px rgba(58,67,227,0.12)",
          }}
        >
          <strong style={{ color: "#588dff" }}>{data.totalConversionPct.toFixed(1)}%</strong> conversão total
        </div>
      </div>
          ...
          {data.stages.map((stage, i) => {
            ...
            return (
              ...
              >
                ...
                />
                <span
                  className="flex h-9 w-9 shrink-0 items-center justify-center rounded-none border"
