    ...
    <div className={`relative flex flex-col items-center gap-2.5 ${m.order}`}>
      ...
      {photo ? (
        <div className={`overflow-hidden rounded-full border-[3px] bg-canvas ${m.ring} ${m.avatarSize}`}>
          <Image src={photo} alt={closer.name} width={96} height={96} className="h-full w-full object-cover" />
        </div>
      ) : (
        <div
      ...
      <p className="max-w-[110px] truncate text-[13px] font-bold text-primary">{closer.name.split(" ")[0]}</p>
      <p className="text-xl font-black text-accent-light">{fmtBRLCompact(closer.mrr)}</p>
...
export function ClosersPodium({ closers }: { closers: CloserStats[] }) {
  ...
  const [c1, c2, c3] = closers;

  return (
    <PodiumShell title="Pódio Closers">
